...
def draw_vector_overlay(ghosts, targets, player_center, player_dir):
    ...
    for idx, g in enumerate(ghosts):
        ...
        for d, allowed in enumerate(getattr(g, 'turns', [False, False, False, False])):
            if not allowed:
                continue
            nx, ny = next_by_dir[d]
            dval = _euclid((nx, ny), (tx, ty))
            options.append((d, nx, ny, dval))
        for d, nx, ny, dval in options:
            text = font.render(f'{int(dval)}', True, color)
    ...
    try:
        ...
        if player_dir == 0:  # right
            ...
            pred = (px - ahead * (num2), py)
        elif player_dir == 2:  # up
            pred = (px, py - ahead * (num1))
        else:  # down
            pred = (px, py + ahead * (num1))
        # desenha previsão em roxo
        pygame.draw.circle(screen, (180, 0, 255), (int(pred[0]), int(pred[1])), 6, 2)
        txt = font.render('Alvo previsto Pinky', True, (180, 0, 255))
        screen.blit(txt, (int(pred[0]) + 8, int(pred[1]) - 8))
    except Exception:
        pass
